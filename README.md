...
[![v9.26.0](https://img.shields.io/badge/version-v9.26.0-brightgreen)](#status)
[![102 tools](https://img.shields.io/badge/tools-102-blue)](#tools)
[![341 tests](https://img.shields.io/badge/tests-341%20passing-success)](#requirements)
...
| **v9.25.0** | ✅ Two consistency fixes for rev18: (1) **cast result pin canonical name** (rev18 ISSUE-1) — `add_cast` and `get_blueprint` now ALWAYS return the same name for the cast result pin (`"AsBP_TunnelCam_C"` — keeps `_C`, no spaces), regardless of whether `compile_blueprint` happened in between. UE's natural `K2Node_DynamicCast::AllocateDefaultPins` regenerates the spaced display form (`"As BP Tunnel Cam"`) on every `ReconstructNode`; v9.25 derives a canonical name from `TargetType->GetName()` on both serialize AND lookup paths, so the LLM never sees pin-name drift across compiles. `connect_pins` / `disconnect_pins` / `set_pin_default` all accept BOTH forms. (2) **`read_log_capture(since_seq=N)` + `latest_seq` response field** (rev18 ISSUE-2) — each captured entry has a monotonic int64 sequence number (survives `clear_log_capture`). Recommended pattern: `baseline = read_log_capture()["latest_seq"]` → do action → `read_log_capture(since_seq=baseline)` gets ONLY new entries. Stops stale failed-compile errors from earlier iterations bleeding into current snapshots |
| **v9.26.0** | ✅ Seven new tools closing rev19's three missing capabilities for the next user task (G/H-key Timeline-triggered movement on duplicated BPs in a new level): **Timeline trio** (`add_timeline` / `add_timeline_track_float` / `set_timeline_curve_key`) — spawns `K2Node_Timeline` + `UTimelineTemplate` + per-track `UCurveFloat` keyframes (linear / cubic / constant interpolation; replace-in-place at duplicate times). Smoke: BP with timeline + 2 float tracks + 3 keys compiles `up_to_date`; `<TrackName>` output pins appear via `ReconstructNode` after each track add. **`duplicate_blueprint`** — `UEditorAssetLibrary::DuplicateAsset` wrapper, refuses overwrites, dup carries the whole Timeline. **Level management** (`create_level` / `load_level` / `list_levels`) — `ULevelEditorSubsystem`-based (UE 5.x API). `load_level` pre-calls `SaveCurrentLevel` so unsaved-changes modal doesn't freeze game thread; `list_levels` flags the active world via Asset Registry `FARFilter`. Module dep: added `"LevelEditor"` to Build.cs |
| **Unit tests** | **341 passing**, 10 integration tests gated on a running UE editor (GUI 10/10, headless 8/10 + 2 explicit skips) |
